...
from hikvision.functionCode.MvCameraControl_class import *
from hikvision.functionCode.MvErrorDefine_const import *
from hikvision.functionCode.CameraParams_header import *
from hikvision.functionCode.PixelType_header import *
from hikvision.utilty.pic_data import *
...
def set_Value(cam, param_type="int_value", node_name="PayloadSize", node_value=None):
    ...
    if param_type == "int_value":
        ...
        ret = cam.MV_CC_SetCommandValue(node_name)
        if ret != 0:
            print("设置command 型数据节点 %s 失败！ 报错码 ret[0x%x]" %(node_name,ret))
            sys.exit()
        # print("设置command 型数据节点 %s 成功！ " %(node_name))
...
def close_destroyhandle_device(cam,data_buf=None):
    # 关闭设备
...
def access_get_video(cam,active_way="getImagebuffer"):
    if active_way == "getoneframetimeout":
        # stParam = MVCC_INTVALUE_EX()
        # memset(byref(stParam), 0, sizeof(MVCC_INTVALUE_EX))
        # ret = cam.MV_CC_GetIntValueEx("PayloadSize", stParam)
        # if ret != 0:
...
def test_record():
    ...
    kjh0 = access_get_video(cam, "getoneframetimeout")

    import time
    time.sleep(60)
    stop_grab_device(cam)
    close_destroyhandle_device(cam)
