...
def operation_camera():
    ...
    # 设置曝光时间 5000
    try:
        set_Value(cam,param_type="float_value",node_name="ExposureTime",node_value=50000)
    except:
        pass
    # 设置触发延迟
    # try:
...
def go_around(cam,slave,x_origin,y_origin,z_origin,which_time_count):
    #x_origin,y_origin,z_origin = -108.337,-68.125,7.397
    images_list = []
    # kk_list = [27.034,27.076,27.118,27.160,27.202,27.244,27.286]
    kk_list = [30.028]
    # kk_list = [27.076,30.028,31.028]
    path_lines = PathPlanning(x_blank=12.175,y_blank=11.48,width=6,height=9,flag=1).get_path_planning()
    # start = time.time()
    image_count = 0
    flag = 0
    for x_xd,y_xd in path_lines:
        ...
        x_this = x_this/1000
        y_this = y_this/1000
        for z_this in kk_list:
            flag += 1
            # b = time.time()
            # print("计算时间{}".format(b-a))
            # print("目标位置 x:{},y:{}".format(x_this,y_this))
            slave.go_straight(x_this,y_this,z_this,None)
            # a = time.time()
            while True:
                x,y,z = get_x_y_z(slave1)
                # print("目标位置 x:{},y:{},z:{},目前位置x:{},y:{},z:{}".format(x_this,y_this,z_this,x,y,z))
                if math.isclose(x_this,x,rel_tol=5e-5) and math.isclose(y_this,y,rel_tol=5e-5) and math.isclose(z_this,z,rel_tol=5e-5):
                    ...
                    # image_name = time_now+".jpg"
                    image_name = str(image_count)+'_'+str(which_time_count)+"_"+str(flag)+".png"

                    if flag == len(kk_list):
                        flag = 0
                    image_path = os.path.join("D:/Projects/motor20210821/images_saves/y3",image_name)
                    if kjh0 is not None:
                        # print('kjh0',kjh0.shape)
                        # a = time.time()
                        # kjh0 = cv2.resize(kjh0,(640,480))
                        cv2.imwrite(image_path,kjh0)
                        # a = time.time()
                        # images_list.append(kjh0)
                        # b = time.time()
                        # print("添加时间{}".format(b-a))
                        # b = time.time()
                        # print("写入文件时间为{}".format(b-a))
                        # time.sleep(0.1)
                        break
                # break
    # end = time.time()
...
if __name__ == '__main__':
    ...
    while True:
        ...
        go_first_position(slave1,10.273, 10.128, 25)#27.334#26.576
        go_around(camera,slave1,10.273, 10.128, 31.028,which_time_count=which_time_count)
