...
from utilty.process_image_neiwu import process_neiwu
...
class DaoGuiAndCamera(QObject):
    ...
    def __init__(self,usb_port:str,addr:str,start_speed:int,end_speed:int,runing_speed:int,acceleration:int):
        ...
        self.running_speed = runing_speed  # 1600
    ...
    def init_camera(self):
        try:
            self.camera1 = self.operation_camera(camera_index=0,exposuretime=5000)
        except:
            pass
    ...
    def go_around_(self,camera,x,y,z,flag):
        self.go_around(camera,self.slave1,x_origin=x,y_origin=y,z_origin=z,path_lines=self.path_lines,images_list=self.images_list,flag=flag)
    ...
    def go_around(self,cam, slave, x_origin, y_origin, z_origin, path_lines, images_list,flag=1):
        child_images_list = []
                ...
                if math.isclose(x_this, x, rel_tol=5e-5) and math.isclose(y_this, y, rel_tol=5e-5):
                    ...
                    if kjh0 is not None:
                        if flag==1:
                            ...
                            kjh0 = process_neiwu(kjh0,scale=0.5,min_area=80000,max_area=200000)
                            if kjh0 is not None:
                                child_images_list.append(kjh0)
                            else:
                                child_images_list.append(np.zeros((150,150,3)))
                    if flag  == 1:
                        if count_flag == 4:
    ...
    def operation_camera(self,camera_index=0,exposuretime=5000):
        # 枚举设备
...
class Mainwindow(QMainWindow):
    ...
    def Work1(self):
        try:
            self.daoguiandcamera = DaoGuiAndCamera("COM4","01",1600,1600,20000,80)  #创建线程对象
    ...
    def Work2(self):
        ...
        try:
            ...
            self.daoguiandcamera.choose_path(10.5, 10.5, 10, 10, 2)
            self.daoguiandcamera.go_origin(self.daoguiandcamera.slave1)
            self.daoguiandcamera.go_first_position_(22.242, 2.867, 23.82)
            self.daoguiandcamera.go_around_(self.daoguiandcamera.camera1,22.242,2.867,23.82,1)
        except:
            pass
    ...
    def Work3(self):
        # 内污的拍摄线程
        self.daoguiandcamera.images_list = []
        self.daoguiandcamera.choose_path(10.4,10.5,10,10,1)
        self.daoguiandcamera.go_origin(self.daoguiandcamera.slave1)
        self.daoguiandcamera.go_first_position_(22.242, 2.867, 23.82)
        self.daoguiandcamera.go_around_(self.daoguiandcamera.camera1,22.242,2.867,23.82,2)
    ...
    def start_check_neiwu(self):
        try:
            self.thread2 = threading.Thread(target=self.Work3, args=())
            self.thread2.start()
        except:
            pass
